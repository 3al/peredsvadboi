<?php
// $Id: template.php,v 1.3 2011/01/06 03:58:38 pixture Exp $
...
//theme('tcompactmenu_block', array('urls' => $entries)); 
function _mayo_get_related_html($items, $nid, $label){
    
    $output = "<span class='related_label'>".$label.":</span>";
    ...
    foreach($items as $num => $item){
            ...
            //если это наша страница, то надо вывести ссылки на 2 следующие
            if($item['nid'] == $nid){
                for($i=$num+1; $i < $num+3;$i++){
                    
                    if(isset($items[$i])){
                        if($count < 2 && !in_array($items[$i]['nid'], $used)){
                            $output.= "<span class='related_link'><img src='/sites/default/files/bullet_red.png'/>".$items[$i]['link']."</span>";
                            $used[] = $items[$i]['nid'];
                            $count++;
                         }  
                    }
                        ...
                        //говнокод, переписать
                       if(isset($items[0]) && $count < 2 && $items[0]['nid'] != $nid && !in_array($items[0]['nid'], $used)){
                            
                            /* var_dump($nid);
                            var_dump($count); exit; */
                            
                            $output.= "<span class='related_link'><img src='/sites/default/files/bullet_red.png'/>".$items[0]['link']."</span>";
                            $used[] = $items[0]['nid'];
                            $count++;
                       }
                       if(isset($items[1]) && $count < 2 && $items[1]['nid'] != $nid && !in_array($items[1]['nid'], $used)){
                            $output.= "<span class='related_link'><img src='/sites/default/files/bullet_red.png'/>".$items[1]['link']."</span>";
                            $used[] = $items[1]['nid'];
                            $count++;
                       }
                        if(isset($items[2]) && $count < 2 && $items[2]['nid'] != $nid && !in_array($items[2]['nid'], $used)){
                            $output.= "<span class='related_link'><img src='/sites/default/files/bullet_red.png'/>".$items[2]['link']."</span>";
                            $used[] = $items[2]['nid'];
                            $count++;
                       }
                    }
 ...
 */
 /*
function mayo_item_list_spec($vars) {
...
//trial
function mayo_node_view_alter(&$build){
    
   
   /*  echo "<pre>";
    var_dump($build['#node']->nid);echo "</pre>"; exit; */
    
   $build['body']['related_links'] = '';
      
    if(
        $build['#view_mode'] !== 'full' || 
        !node_is_page($build['#node']) ||
        !isset($build['#node']->field_cat['ru'][0]['tid']) ||
        !count($build['#node']->field_cat['ru'])
    ){
        return false;
    }
    
    $node = $build['#node'];
    $catid = $build['#node']->field_cat['ru'][0]['tid'];
    $items = array();
    ...
    foreach ($nodes as $node1){
            $items[$i]['link'] = l($node1->title, 'node/' . $node1->nid);
            $items[$i]['nid'] = $node1->nid;
            $i++;
    }
   ...
   /*  echo "<pre>";
    ...
    if(count($items)){
       $build['body']['related_links'] = _mayo_get_related_html($items, $node->nid,'Следующие статьи');
    }
}
